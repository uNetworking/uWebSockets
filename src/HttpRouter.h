...
private:
    ...
    /* List of handlers */
    std::vector<ofats::any_invocable<bool(HttpRouter *)>> handlers;
...
public:
    ...
    void add(std::vector<std::string> methods, std::string pattern, ofats::any_invocable<bool(HttpRouter *)> &&handler, uint32_t priority = MEDIUM_PRIORITY) {
        for (std::string method : methods) {
            /* Lookup method */
