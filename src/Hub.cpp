...
void Hub::connect(const std::string& uri, void *user, const HttpHeaderMap&  extraHeaders, int timeoutMs, Group<CLIENT> *eh) {
    if (!eh) {
...
void Hub::ontHttpOnlyConnection(uS::Socket *s, bool error)
{
	HttpSocket<CLIENT> *httpSocket = (HttpSocket<CLIENT> *) s;
...
HttpSocket<CLIENT>* Hub::HttpSocketConnect(const std::string& uri, void *user, Group<CLIENT> *eh, std::string& hostname, std::string& path)
{
	if (!eh) {
		eh = (Group<CLIENT> *) this;
	}
...
*/
void Hub::connect(const std::string& uri, HttpMethod method, void *user,
	...
	int timeoutMs, Group<CLIENT> *eh)
{
	...
	HttpSocket<CLIENT> *httpSocket = HttpSocketConnect(uri, user, eh, hostname, path);
